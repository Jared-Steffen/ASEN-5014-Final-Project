function Plot_Outputs(tspan,output_vecs,ref_inputs_vecs,subplot_title)
    ...
    yline(0.2*1.1,'-','10% Overshoot','LabelVerticalAlignment','bottom')
    yline(0.2*0.95,'-','95% Desired Reference','LabelVerticalAlignment','top')
    xline((5340+3600)/3600,'-','1 Hour After Step Input','LabelVerticalAlignment','bottom')
    xlabel('Time [hr]')
    ...
    yline(1e-4*1.1,'-','10% Overshoot','LabelVerticalAlignment','bottom')
    yline(1e-4*0.95,'-','95% Desired Reference','LabelVerticalAlignment','bottom')
    xline((5340+3600)/3600,'-','1 Hour After Step Input','LabelVerticalAlignment','bottom')
    xlabel('Time [hr]')
    ylabel('$\delta \theta$ [rad]','Interpreter','latex')
